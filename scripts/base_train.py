...
parser.add_argument("--routing-loop", action="store_true", help="enable the two-pass causal routing-feedback model")
parser.add_argument("--routing-pattern", type=str, default="progressive", help="last|same|reverse|progressive|offset:<fraction-or-layers>")
parser.add_argument("--routing-gate-init", type=float, default=0.05, help="initial additive routing-logit strength")
parser.add_argument("--routing-detach", action="store_true", help="stop pass-2 gradients through pass-1 routing Q/K")
parser.add_argument("--first-pass-loss-weight", type=float, default=0.1, help="auxiliary LM loss weight on pass 1")
...
def build_model_meta(depth):
    ...
    config = GPTConfig(
        ...
        routing_loop=args.routing_loop, routing_pattern=args.routing_pattern,
        routing_gate_init=args.routing_gate_init, routing_detach=args.routing_detach,
        first_pass_loss_weight=args.first_pass_loss_weight,
    )
...
if args.routing_loop:
    print0("Routing loop enabled: two shared-weight causal passes; pass 2 uses pass-1 deep Q/K as an additive attention prior.")
